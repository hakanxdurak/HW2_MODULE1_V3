...
#pragma HLS PIPELINE II = 1
	...
	float target_array[GPR_SIZE];
	float x_array[GPR_SIZE];
	for(int i = 0 ; i < GPR_SIZE ; i++){
#pragma HLS PIPELINE II = 1
		local_read_target = TARGET.read();
		target_array[i] = local_read_target.data;
		local_read_x = X.read();
		x_array[i] = local_read_x.data;
	}
	float norms;
	float b;
	float MSXHt[W_SIZE];
...
#pragma HLS PIPELINE II = 1
		...
		for(int i1 = 0 ; i1 < H_SIZE ; i1++){
			target_tmp = i0 + (i1 << 8);
			norms = norms + target_array[target_tmp] * h_array[i1];
			b = b + x_array[target_tmp] * h_array[i1];
		}
		if(norms > b)
			MSXHt[i0] = 0.0;
		else
